...
function renderCalculation(q){
  ...
  var opts=q.opts.map(function(o,i){
    return '<button class="calc-opt" id="copt'+i+'" onclick="pickCalc('+i+')">'+'<span class="calc-letter">'+String.fromCharCode(65+i)+'</span>'+'<span class="calc-ped">'+o.ped+'</span>'+'<span><div class="calc-type-lbl">'+o.typ+'</div><div class="calc-rev">'+o.rev+'</div></span>'+'</button>';
  }).join('');
...
function renderDiagnosticPair(q){
  var cards=q.students.map(function(st,i){
    return '<div class="pair-card" id="pc'+i+'" onclick="pickPair('+i+')">'+'<div class="pair-header"><div class="pair-name">'+st.name+'</div><span class="pair-badge">'+st.badge+'</span></div>'+'<div class="pair-body">'+st.answer+'</div></div>';
  }).join('');
  r(qHeader(q)+'<div class="q-stem">'+q.stem+'</div>'+'<p class="pair-instr">Read both answers carefully, then tap the one you think would score more highly in an Edexcel exam.</p>'+'<div class="pair-grid">'+cards+'</div>'+'<div id="fb"></div><div id="nr" class="next-row"></div>');
}
function pickPair(i){
  if(S.answered) return; S.answered=true;
  var q=Qs[S.qi]; var ok=(i===q.ans);
...
function renderMCQ(q){
  var opts=q.opts.map(function(o,i){
    return '<button class="mcq-opt" id="mo'+i+'" onclick="pickMCQ('+i+')">'+'<span class="opt-letter">'+String.fromCharCode(65+i)+'</span>'+'<span class="opt-text">'+o+'</span></button>';
  }).join('');
  r(qHeader(q)+'<div class="q-stem">'+q.stem+'</div>'+'<div class="mcq-opts">'+opts+'</div>'+'<div id="fb"></div><div id="nr" class="next-row"></div>');
}
function pickMCQ(i){
  if(S.answered) return; S.answered=true;
  var q=Qs[S.qi]; var ok=(i===q.ans);
...
function renderConfidenceMCQ(q){
  ...
  var opts=q.opts.map(function(o,i){
    return '<button class="mcq-opt" id="cmo'+i+'" onclick="pickConfAnswer('+i+')">'+'<span class="opt-letter">'+String.fromCharCode(65+i)+'</span>'+'<span class="opt-text">'+o+'</span></button>';
  }).join('');
  var confPanel='<div class="conf-panel" id="conf-panel">'+'<div class="conf-q">Now - how confident were you in that answer?</div>'+'<div class="conf-btns">'+'<button class="conf-btn conf-certain" onclick="pickConfLevel(\'certain\')">&#10003; Very sure</button>'+'<button class="conf-btn conf-unsure" onclick="pickConfLevel(\'unsure\')">~ Fairly sure</button>'+'<button class="conf-btn conf-guess" onclick="pickConfLevel(\'guess\')">? Just guessing</button>'+'</div></div>';
  r(qHeader(q)+'<div class="q-stem">'+q.stem+'</div>'+'<div class="mcq-opts">'+opts+'</div>'+confPanel+'<div id="fb"></div><div id="nr" class="next-row"></div>');
}
...
function renderOddOneOut(q){
  var cards=q.items.map(function(it,i){
    return '<button class="oo-card" id="oo'+i+'" onclick="pickOOO('+i+')">'+'<div class="oo-icon">'+it.icon+'</div>'+'<div class="oo-label">'+it.label+'</div>'+'<div class="oo-note">'+it.note+'</div></button>';
  }).join('');
  r(qHeader(q)+'<div class="q-stem">'+q.stem+'</div>'+'<div class="oo-grid">'+cards+'</div>'+'<div id="fb"></div><div id="nr" class="next-row"></div>');
}
function pickOOO(i){
  if(S.answered) return; S.answered=true;
  var q=Qs[S.qi]; var ok=(i===q.ans);
...
function renderMultiSelect(q){
  ...
  var opts=q.opts.map(function(o,i){
    return '<button class="ms-opt" id="ms'+i+'" onclick="toggleMS('+i+')">'+'<div class="ms-box" id="msb'+i+'"></div>'+'<span class="ms-opt-text">'+String.fromCharCode(65+i)+'. '+o+'</span></button>';
  }).join('');
  r(qHeader(q)+'<div class="q-stem">'+q.stem+'</div>'+'<p class="ms-instr">Tick all that apply. Select as many or as few as you think are correct.</p>'+'<div class="ms-opts">'+opts+'</div>'+'<div style="text-align:center;margin-bottom:.9rem"><button class="btn b-submit" id="ms-sub" onclick="submitMS()">Check my selection</button></div>'+'<div id="fb"></div><div id="nr" class="next-row"></div>');
}
function toggleMS(i){
  if(S.answered) return;
...
function renderRankInner(q, instr, mostLbl, leastLbl){
  ...
  var cards=display.map(function(origIdx){
    ...
    return '<button class="rank-item" id="ri'+origIdx+'" onclick="tapRank('+origIdx+')">'+'<div class="rank-badge" id="rb'+origIdx+'">?</div>'+'<div class="rank-label">'+it.label+'</div>'+'<div class="rank-note">'+it.note+'</div></button>';
  }).join('');
  r(qHeader(q)+'<div class="q-stem">'+q.stem+'</div>'+'<div class="rank-instr">'+instr+'</div>'+'<div class="rank-tracker"><span class="rank-tracker-lbl">Your order:</span>'+slots+'</div>'+'<div style="display:flex;justify-content:space-between"><span class="rank-most-lbl">'+mostLbl+'</span><span class="rank-least-lbl">'+leastLbl+'</span></div>'+'<div class="rank-grid">'+cards+'</div>'+'<div style="text-align:center;margin-bottom:.9rem"><button class="btn b-submit" id="rank-sub" onclick="submitRank()" disabled>Check my answer</button></div>'+'<div id="fb"></div><div id="nr" class="next-row"></div>');
}
...
function renderParaFill(q){
  ...
  r(qHeader(q)+'<div class="q-stem">'+q.stem+'</div>'+'<div class="para-box">'+anchor+para+'</div>'+'<div class="pf-controls">'+rows+'</div>'+'<div class="pf-submit-row"><button class="btn b-submit" id="pf-sub" onclick="submitPF()" disabled>Check my paragraph</button></div>'+'<div id="fb"></div><div id="nr" class="next-row"></div>');
}
...
function renderDataTable(q){
  ...
  var opts=q.opts.map(function(o,i){
    return '<button class="mcq-opt" id="dmo'+i+'" onclick="pickDT('+i+')">'+'<span class="opt-letter">'+String.fromCharCode(65+i)+'</span>'+'<span class="opt-text">'+o+'</span></button>';
  }).join('');
  r(qHeader(q)+'<div class="q-stem">'+q.stem+'</div>'+'<div class="tbl-wrap"><table><thead>'+thead+'</thead><tbody>'+tbody+'</tbody></table></div>'+'<div class="q-stem" style="margin-bottom:1rem">'+q.question+'</div>'+'<div class="mcq-opts">'+opts+'</div>'+'<div id="fb"></div><div id="nr" class="next-row"></div>');
}
function pickDT(i){
  if(S.answered) return; S.answered=true;
  var q=Qs[S.qi]; var ok=(i===q.ans);
...
function renderDiagramInterp(q){
  var opts=q.opts.map(function(o,i){
    return '<button class="mcq-opt" id="dimo'+i+'" onclick="pickDIM('+i+')">'+'<span class="opt-letter">'+String.fromCharCode(65+i)+'</span>'+'<span class="opt-text">'+o+'</span></button>';
  }).join('');
  r(qHeader(q)+'<div class="q-stem">'+q.stem+'</div>'+'<div class="diag-box">'+q.svg+(q.caption?'<div class="diag-caption">'+q.caption+'</div>':'')+'</div>'+'<div class="q-stem" style="margin-bottom:1rem">'+q.question+'</div>'+'<div class="mcq-opts">'+opts+'</div>'+'<div id="fb"></div><div id="nr" class="next-row"></div>');
}
function pickDIM(i){
  if(S.answered) return; S.answered=true;
  var q=Qs[S.qi]; var ok=(i===q.ans);
